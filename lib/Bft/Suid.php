<?php
/**
 * Short Unique id ('suid') Class.
 *
 * Not sure why I wrote this, but the idea is it's a short alternative to UUIDs
 * Contains 6 bytes (48-bits) packed into 8 characters, matching ^[_a-zA-Z0-9.]{8}$
 * We could, of course, use BASE64-encoding, but this method avoids characters that
 * need special URL encoding, such as '+', '/' and '='.
 * 
 * There are about 2.8 x 10^14 different SUIDs available (> 280 thousand billion). For
 * most purposes, the chances of collision are vanishingly small.
 * 
 * Examples:
 * <code>
 *  
...
class Bft_SUID
{
    ...
    public function str()
    {
        if (null === $this->_str) {
            ...
            $this->_str  = substr(Bft_SUID::b64chars, ((($ints[1])&(0xfc00))>>10), 1);
            $this->_str .= substr(Bft_SUID::b64chars, ((($ints[1])&(0x03f0))>>4), 1);
            $this->_str .= substr(Bft_SUID::b64chars, (((($ints[1])&(0x000f))<<2) | ((($ints[2])&(0xc000))>>14)), 1);
            $this->_str .= substr(Bft_SUID::b64chars, ((($ints[2])&(0x3f00))>>8), 1);
            $this->_str .= substr(Bft_SUID::b64chars, ((($ints[2])&(0xfc))>>2), 1);
            $this->_str .= substr(Bft_SUID::b64chars, (((($ints[2])&(0x3))<<4) | ((($ints[3])&(0xf000))>>12)), 1);
            $this->_str .= substr(Bft_SUID::b64chars, ((($ints[3])&(0xfc0))>>6), 1);
            $this->_str .= substr(Bft_SUID::b64chars, (($ints[3])&(0x3f)), 1);
        }
        return $this->_str;
    }
    ...
    {
        if (Bft_SUID::$ords === null) {
            Bft_SUID::$ords = array();
            for ($i=0; $i<64; ++$i) {
                $char = substr(Bft_SUID::b64chars, $i, 1);
                Bft_SUID::$ords[$char] = $i;
            }
        }
        return Bft_SUID::$ords[$chr];
    }
